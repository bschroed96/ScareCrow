...
func JScript_Buff(fso string, dropPath string, encoded string, code string, name string, mode string, sandbox bool, encoding string, outDir string) string {
	var buffer bytes.Buffer
	JScript := &JScript{}
	...
	buffer.Reset()
	JSTemplate, err := template.New("JScript").Parse(Struct.JSfile(encoding, outDir))
	if err != nil {
		log.Fatal(err)
	}
	buffer.Reset()
...
func CompileLoader(mode string, outFile string, filename string, name string, CommandLoader string, URL string, sandbox bool, encoding string, outDir string) {
	if mode == "excel" {
	...
	finalcode := JScript_Buff(fso, dropPath, encoded, code, name, mode, sandbox, encoding, outDir)
